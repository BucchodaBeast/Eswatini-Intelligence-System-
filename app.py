...
_CONVERGENCE_STOPWORDS = {
    'this', 'that', 'these', 'those', 'with', 'from', 'about', 'have', 'been',
    'were', 'will', 'shows', 'show', 'shown', 'data', 'report', 'reports',
    'according', 'while', 'compared', 'against', 'over', 'under', 'into',
    'than', 'also', 'more', 'less', 'both', 'each', 'such', 'some', 'other',
    'which', 'their', 'there', 'here', 'when', 'where', 'what', 'suggest',
    'suggests', 'suggesting', 'signal', 'signals', 'source', 'sources',
    'watch', 'watcher', 'flag', 'flagged', 'flagging', 'recent', 'recently',
}

def _tokenize_for_convergence(text):
    words = _re.findall(r"[a-zA-Z][a-zA-Z0-9\-']{2,}", (text or '').lower())
    return [w for w in words if w not in _CONVERGENCE_STOPWORDS]

def _compute_corpus_df(posts):
    """Document frequency for each token across the current convergence
    window — the basis for telling 'domain vocabulary' apart from a real
    shared signal. A term every third post uses isn't a coincidence worth
    flagging; a term only two posts share probably is."""
    df = defaultdict(int)
    for post in posts:
        for t in set(_tokenize_for_convergence(post.get('body', '') or '')):
            df[t] += 1
    return df

def _extract_weighted_terms(text, df, N, max_df_ratio=0.4):
    """Candidate convergence terms, filtered by rarity. A term appearing in
    more than max_df_ratio of the current window's posts (e.g. 'sacu',
    'world bank' in a regional-economics system) is domain vocabulary, not
    a meaningful shared signal — exclude it rather than let it false-fire
    convergence on every pair of posts that happen to share the subject."""
    if N == 0:
        return set()
    tokens = set(_tokenize_for_convergence(text))
    return {t for t in tokens if df.get(t, 0) > 0 and (df[t] / N) <= max_df_ratio}
...
def check_convergence():
    ...
    # ── PASS 1: Rarity-weighted convergence (precise, not tag-noise) ─────────
    # Regex entities (tickers, CVEs, patents) are inherently rare even if
    # repeated, so they're kept unconditionally. Everything else has to earn
    # its place by being uncommon in the current window — see
    # _extract_weighted_terms.
    corpus_df = _compute_corpus_df(recent)
    N         = len(recent)
    ...
    for post in recent:
        ...
        entities |= _extract_weighted_terms(body, corpus_df, N)
    ...
    generic_topics = {'#eswatini', '#SACU', '#trade', '#macro', '#regional'}
...
_STANCE_POSITIVE = {
    'improve', 'improving', 'improved', 'improvement', 'growth', 'growing',
    'grew', 'stable', 'stability', 'resilient', 'resilience', 'strong',
    'strengthen', 'strengthening', 'recovery', 'recovering', 'recovered',
    'gain', 'gains', 'surplus', 'increase', 'increasing', 'increased',
    'rise', 'rising', 'rose', 'expand', 'expanding', 'expansion', 'orderly',
}
_STANCE_NEGATIVE = {
    'decline', 'declining', 'declined', 'deficit', 'shortage', 'shortfall',
    'crisis', 'risk', 'risks', 'concern', 'concerning', 'concerns',
    'warning', 'drop', 'dropping', 'dropped', 'fall', 'falling', 'fell',
    'weak', 'weakening', 'weakness', 'stress', 'pressure', 'pressures',
    'struggle', 'struggling', 'below', 'deteriorate', 'deteriorating',
    'contraction', 'downgrade', 'downgraded', 'exposure', 'vulnerable',
    'tightening', 'restriction', 'restrictions',
}

def _infer_stance(text):
    """Cheap keyword-polarity read on what a post is actually concluding —
    not real sentiment analysis, but enough to tell 'things look fine'
    apart from 'things look strained', which is the real bar for a Town
    Hall to mean something other than 'two agents mentioned the same tag'."""
    text = (text or '').lower()
    pos = sum(1 for w in _STANCE_POSITIVE if w in text)
    neg = sum(1 for w in _STANCE_NEGATIVE if w in text)
    if pos == 0 and neg == 0:
        return 'neutral'
    if pos == neg:
        return 'mixed'
...
def create_town_hall(citizen_a, post_a, citizen_b, post_b, topic_tag, stance_a='signals', stance_b='counter'):
    """Generate a Town Hall debate post from two genuinely conflicting
    agent posts — stance_a/stance_b should already differ (checked by the
    caller); this just renders honest labels instead of placeholder text."""
    import uuid
    th = {
        'id':        str(uuid.uuid4()),
        'type':      'town_hall',
        ...
        'positions': [
            {
                'citizen': citizen_a,
                'stance':  stance_a.capitalize(),
                'text':    (post_a.get('body', '') or '')[:300],
            },
            {
                'citizen': citizen_b,
                'stance':  stance_b.capitalize(),
...
def check_for_disagreement():
    ...
    for citizen_a, citizen_b in DIVERGENT_PAIRS:
        ...
        if not _posts_are_semantically_related(post_a, post_b):
            ...
            continue
        # Stance filter: verify they're actually saying different things,
        # not just both posting calmly about the same topic. This is the
        # difference between a real divergence and a relabeled convergence.
        stance_a = _infer_stance(post_a.get('body', ''))
        stance_b = _infer_stance(post_b.get('body', ''))
        if stance_a == stance_b:
            log.debug(f"Skipping {citizen_a} vs {citizen_b} on {topic_tag} — same stance ({stance_a}), not a real divergence")
            continue
        log.info(f"DISAGREEMENT: {citizen_a} ({stance_a}) vs {citizen_b} ({stance_b}) on {topic_tag}")
        create_town_hall(citizen_a, post_a, citizen_b, post_b, topic_tag, stance_a, stance_b)
