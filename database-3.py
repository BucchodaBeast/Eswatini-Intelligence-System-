...
class SQLiteDB:
    ...
    def init(self):
        with self.conn() as c:
            c.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS council_subpoenas (
                    id              TEXT PRIMARY KEY,
                    agent           TEXT NOT NULL,
                    question        TEXT,
                    session_id      TEXT,
    ...
    def create_subpoena(self, agent, question, session_id=None):
        """Issue a Council subpoena — an unresolved question a specific
        agent should address on its next run. Nothing calls this yet (no
        automatic issuance is wired up), but the read/resolve side needs
        it to exist and work correctly."""
        sid = str(uuid.uuid4())
        with self.conn() as c:
            c.execute(
                "INSERT INTO council_subpoenas (id, agent, question, session_id, issued_at, resolved) "
                "VALUES (?, ?, ?, ?, ?, 0)",
                (sid, agent, question, session_id, datetime.utcnow().isoformat()),
            )
        return sid

    def get_pending_subpoenas_for_agent(self, agent):
        with self.conn() as c:
            rows = c.execute(
                "SELECT * FROM council_subpoenas WHERE agent=? AND resolved=0 ORDER BY issued_at ASC",
                (agent,),
            ).fetchall()
        return [dict(r) for r in rows]

    def resolve_subpoenas_for_agent(self, agent):
        with self.conn() as c:
            c.execute(
                "UPDATE council_subpoenas SET resolved=1 WHERE agent=? AND resolved=0",
                (agent,),
            )
    ...
    def get_council_session_for_condition(self, condition_id, hours=12):
        """Dedup check for COUNCIL_CONDITIONS — has a session already been
        triggered for this condition within the rate-limit window? Sessions
        are linked to a condition via the '#condition-{id}' tag embedded in
        the JSON tags column (condition_id itself isn't a stored column)."""
        since = (datetime.utcnow() - timedelta(hours=hours)).isoformat()
        needle = f'"#condition-{condition_id}"'
        with self.conn() as c:
            row = c.execute(
                "SELECT * FROM council_sessions WHERE created_at > ? AND tags LIKE ? "
...
class SupabaseDB:
    ...
    def create_subpoena(self, agent, question, session_id=None):
        try:
            row = {
                'id': str(uuid.uuid4()), 'agent': agent, 'question': question,
                'session_id': session_id, 'issued_at': datetime.utcnow().isoformat(),
                'resolved': 0,
            }
            self.client.table('council_subpoenas').insert(row).execute()
            return row['id']
        except Exception as e:
            log.error(f"create_subpoena: {e} — does the council_subpoenas table exist in Supabase?")
            return None

    def get_pending_subpoenas_for_agent(self, agent):
        try:
            r = (self.client.table('council_subpoenas').select('*')
                 .eq('agent', agent).eq('resolved', 0)
                 .order('issued_at', desc=False).execute())
            return r.data or []
        except Exception as e:
            log.error(f"get_pending_subpoenas_for_agent: {e} — does the council_subpoenas table exist in Supabase?")
            return []

    def resolve_subpoenas_for_agent(self, agent):
        try:
            (self.client.table('council_subpoenas').update({'resolved': 1})
             .eq('agent', agent).eq('resolved', 0).execute())
        except Exception as e:
            log.error(f"resolve_subpoenas_for_agent: {e}")
    ...
    def get_council_session_for_condition(self, condition_id, hours=12):
        ...
        try:
            r = (self.client.table('council_sessions').select('*')
                 .gte('created_at', since).like('tags', f'%{needle}%')
                 .order('created_at', desc=True).limit(1).execute())
            return r.data[0] if r.data else None
        except Exception as e:
            log.error(f'get_council_session_for_condition: {e}')
            return None
