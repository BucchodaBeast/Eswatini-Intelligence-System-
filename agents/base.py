...
class BaseAgent:
    ...
    def run(self):
        ...
        try:
            ...
            self.log.error(f"fetch_data() raised [{type(e).__name__}]: {e}")
            raw_items = []
        if not raw_items:
            self.log.warning(f"No new data found — fetch_data() returned {raw_items!r}")
            return []
        self.log.info(f"fetch_data() returned {len(raw_items)} item(s)")
        posts = []
        ...
        try:
            resp = requests.post(
                ...
                json={
                    ...
                    'temperature':      0.7,
                    'max_tokens':       700,
                    # gpt-oss models spend part of the token budget on a
                    # hidden reasoning trace before writing the actual
                    # response — at 'medium' (the default) this was eating
                    # enough of max_tokens to truncate the JSON mid-string.
