...
"""
    ...
    def _fetch_nasa_power(self):
        ...
        try:
            ...
            # NASA POWER uses -999 as a missing-data sentinel. The old
            # `v >= 0` precip filter happened to catch it by accident;
            # temp wasn't filtered at all, so one missing day could drag
            # a 35-day average down to something physically absurd.
            valid_precip = [v for v in precip.values() if isinstance(v, (int, float)) and v > -900]
            valid_temp   = [v for v in temp.values()   if isinstance(v, (int, float)) and v > -900]
            if not valid_precip:
                return []
            ...
            return [{
                ...
                'days_with_data':  f"{len(valid_precip)}/{len(precip)} precip, {len(valid_temp)}/{len(temp)} temp",
